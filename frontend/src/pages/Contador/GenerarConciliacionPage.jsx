...
const GenerarConciliacionPage = () => {
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-4">Conciliaciones Generadas</h2>
      <ul className="bg-white rounded shadow p-4">
        {conciliaciones.map((conciliacion) => (
          <li key={conciliacion.id} className="border-b p-2">{conciliacion.fecha} - Estado: {conciliacion.estado.nombre}</li>
        ))}
      </ul>
    </div>
  );
