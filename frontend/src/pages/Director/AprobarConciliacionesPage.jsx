...
const RevisarConciliacionesDirectorPage = () => {
  ...
  const cargarConciliaciones = async () => {
    try {
      ...
      const conciliacionesFiltradas = data.filter(
        (c) => c.estado.id === 2 // ✅ Solo mostrar conciliaciones en estado "En Revisión"
      );
  ...
  const aprobarConciliacion = async (conciliacionId) => {
    try {
      await conciliacionAPI.updateConciliacionEstado(conciliacionId, {
        estadoId: 3, // ✅ Estado "Conciliado"
        directorId: JSON.parse(localStorage.getItem("user")).id,
      });
    ...
    try {
      ...
      const detallesProcesados = data.conciliacionesDetalles.map((detalle) => {
        ...
        return {
          ...
          debe:
            movimiento?.debe !== undefined
              ? parseFloat(movimiento.debe).toFixed(2)
              : libro?.debe !== undefined
              ? parseFloat(libro.debe).toFixed(2)
              : "0.00",
          haber:
            movimiento?.haber !== undefined
              ? parseFloat(movimiento.haber).toFixed(2)
              : libro?.haber !== undefined
              ? parseFloat(libro.haber).toFixed(2)
              : "0.00",
        };
      });
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-4">Revisión de Conciliaciones (Director)</h2>
      ...
      {modalOpen && selectedConciliacion && (
        ...
        >
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-2xl">
            ...
            <p>
              <strong>Cuenta:</strong> {selectedConciliacion.cuenta.nombre}
            </p>
            <p>
              <strong>Fecha:</strong> {new Date(selectedConciliacion.fecha).toLocaleDateString()}
            </p>
            ...
            <div className="max-h-64 overflow-y-auto border rounded">
              {detalles.length > 0 ? (
                ...
                <div className="text-center p-4 text-gray-500">
                  No hay detalles disponibles para esta conciliación.
                </div>
              )}
            </div>
...
export default RevisarConciliacionesDirectorPage;
